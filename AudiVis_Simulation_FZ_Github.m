...
% (submitted to Cognitive Neuroscience) entitled "Reactivation of neural patterns during memory reinstatement supports encoding specificity"
%
...
cond = 1;% controls whether match or mismatch condition is simulated; set to 1 for match, and 2 for mismatch
...
for l=1:length(reactvex);  % loop over reactivation strengths
    ...
    for k=1:nitems;% loop over trials (number of items)
        ...
        R(k,1)=fisherz(corr2(Csem,Rsem));
        R(k,2)=fisherz(corr2(Csens,Rsens));
        Rsens(k,l)=R(k,2);
    end
...
figure;bar(mean(Memper));axis([0 3 0.7 1.3]);
hold on
plot([1 2],Memper);axis([0 3 0 1.3]);
hold off
